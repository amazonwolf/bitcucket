...
@section('content-header')
    <h1>
        {{ trans('deci::decis.title.edit decis') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.deci.decis.index') }}">{{ trans('deci::decis.title.decis') }}</a></li>
        <li class="active">{{ trans('deci::decis.title.edit decis') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.deci.decis.update', $decis->id], 'method' => 'put']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('deci::admin.decis.partials.edit-fields', ['lang' => $locale])
                        </div>
                    @endforeach
